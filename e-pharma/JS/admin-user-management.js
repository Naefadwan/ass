...
function setupUserManagementEvents() {
  ...
  async function fetchUsersFromDatabase() {
    ...
    try {
      ...
      if (!window.AdminAPIService) {
        console.error("👥 ADMIN USERS: AdminAPIService not available");
        return [];
      }
      
      // Use the AdminAPIService to fetch users
      const users = await AdminAPIService.getUsers();
      console.log("👥 ADMIN USERS: Fetched users:", users);
      
      // If the response is not an array, check if it has a data property
      if (!Array.isArray(users)) {
        if (users && users.data && Array.isArray(users.data)) {
          return formatUsers(users.data);
        }
        // If it's an object but not in the expected format, return an empty array
        console.warn("👥 ADMIN USERS: Unexpected response format", users);
        return [];
      }
  ...
  // Format users from the API to match our UI structure
  function formatUsers(users) {
    if (!Array.isArray(users)) {
      console.error("👥 ADMIN USERS: Expected array of users but got:", typeof users);
      return [];
    }
    
    // Map the API user structure to our UI structure
    return users.map(user => ({
      ...
      registeredDate: user.registeredDate || user.created_at || user.createdAt || 'Unknown',
      avatar: user.avatar || user.profilePicture || '../images/avatars/default.jpg'
    }));
  }
